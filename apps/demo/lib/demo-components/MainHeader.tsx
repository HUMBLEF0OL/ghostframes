...
const navItems = [
  ...
  { href: "/slots", label: "Slots", isActive: (pathname: string) => pathname === "/slots" },
  {
    href: "/reference",
    label: "Reference",
    isActive: (pathname: string) => pathname.startsWith("/reference"),
  },
  {
    href: "/config-playground",
    label: "Playground",
    isActive: (pathname: string) => pathname.startsWith("/config-playground"),
  },
  {
    href: "/advanced",
    label: "Advanced",
    isActive: (pathname: string) => pathname.startsWith("/advanced"),
  },
];
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-zinc-800/80 bg-zinc-950/90 backdrop-blur light:border-zinc-200 light:bg-white/90">
      <div className="app-shell flex w-full items-center justify-between gap-3 px-4 py-3 sm:px-6">
        <Link
          href="/"
          className="flex items-center gap-2 rounded-md px-1 py-1 transition-opacity hover:opacity-85"
        >
          <BrandMark size={24} />
          <span className="text-md font-bold text-white tracking-widest light:text-zinc-900">
            GHOSTFRAMES
          </span>
        </Link>
        ...
        <nav
          aria-label="Primary"
          className="flex flex-1 items-center justify-center gap-1 overflow-x-auto px-1"
        >
          {navItems.map((item) => {
