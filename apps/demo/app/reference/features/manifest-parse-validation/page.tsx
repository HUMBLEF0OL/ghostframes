      ...
      >
        <div className="space-y-4 text-sm text-zinc-400 light:text-zinc-700">
          <div className="rounded-lg border border-zinc-800 bg-zinc-950 p-3 light:border-zinc-200 light:bg-zinc-50">
            <p
              data-testid="manifest-parse-success"
              className="font-semibold text-emerald-400 light:text-emerald-700"
            >
              {valid.success ? "Valid manifest accepted" : "Valid manifest rejected"}
            </p>
            <p className="mt-1 text-xs text-zinc-500 light:text-zinc-600">
              Entries: {validEntryCount} | Version: {validManifest?.manifestVersion ?? "unknown"} |
              App: {validManifest?.build.appVersion ?? "unknown"}
            </p>
          </div>

          <div className="rounded-lg border border-zinc-800 bg-zinc-950 p-3 light:border-zinc-200 light:bg-zinc-50">
            <p
              data-testid="manifest-parse-failure"
              className="font-semibold text-rose-400 light:text-rose-700"
            >
              {invalidWrongVersion.success
                ? "Invalid manifest accepted"
                : "Invalid manifest rejected"}
            </p>
            <p
              data-testid="manifest-parse-error-message"
              className="mt-1 text-xs text-zinc-500 light:text-zinc-600"
            >
          ...
          <div className="rounded-lg border border-zinc-800 bg-zinc-950 p-3 light:border-zinc-200 light:bg-zinc-50">
            <p className="font-semibold text-rose-400 light:text-rose-700">
              {invalidMissingDefaults.success
                ? "Invalid defaults accepted"
                : "Invalid defaults rejected"}
            </p>
            <p className="mt-1 text-xs text-zinc-500 light:text-zinc-600">
