  ...
  return (
    <div className="space-y-6">
      <header className="space-y-3 rounded-2xl border border-zinc-800 bg-gradient-to-br from-zinc-900 via-zinc-900 to-indigo-950 p-6 light:border-zinc-200 light:from-white light:via-zinc-50 light:to-indigo-50">
        <p className="text-xs font-semibold uppercase tracking-[0.2em] text-indigo-300 light:text-indigo-700">
        ...
        <p className="max-w-3xl text-zinc-400 light:text-zinc-600">
          This command-first workflow wraps components, captures artifacts, and validates rollout
          readiness with explicit quality gates.
        </p>
      </header>
      ...
      >
        <ol className="space-y-3 text-sm text-zinc-300 light:text-zinc-700">
          <li>
            ...
            Use AutoSkeleton in loading paths.
          </li>
          <li>
            <span className="font-semibold text-white light:text-zinc-900">
              2. Run capture and quality checks.
            </span>
            Generate and verify manifest artifacts in CI.
          </li>
          <li>
            <span className="font-semibold text-white light:text-zinc-900">
              3. Import generated artifacts once.
            </span>
      ...
      <FeatureCard
        title="Capabilities"
        description="Current ghostframes command surface exposed by @ghostframes/runtime"
        badge="ghostframes"
      >
        <div className="grid gap-3 sm:grid-cols-2">
          {capabilityCards.map((card) => (
            ...
            >
              <h2 className="mb-1 text-sm font-semibold text-white light:text-zinc-900">
                {card.title}
              </h2>
      ...
      <FeatureCard
        ...
        description="Generate in build/dev, validate in CI, sanity-check in deploy"
        badge="Build + CI + Deploy"
      >
      ...
      <FeatureCard
        ...
        description="Package scripts used in this repository"
        badge="pnpm"
      >
      ...
      <FeatureCard
        title="Demo App Runbook"
        description="Use this sequence to demonstrate the full workflow locally"
        badge="Demo"
      >
      ...
      <FeatureCard
        title="Runtime Rollout Template"
        description="Environment toggles that drive route-scoped behavior in the demo app"
      >
        <CodeBlock code={rolloutEnvTemplate} language="bash" />
      </FeatureCard>

      <FeatureCard
        title="Direct CLI Invocation"
        description="Command entry points exposed by the installed ghostframes binary"
      >
