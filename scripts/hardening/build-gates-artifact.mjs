...
const GATE_DEFINITIONS = [
  ...
  },
  {
    ...
    description:
      "Blueprint quality gate enforces >= 0.90 threshold and >= 50% malformed fallback reduction",
  ...
  },
  {
    ...
    description:
      "Hybrid confidence gate requires hit/invalidation targets, no user-visible regression, and rollback drill <= 10m",
    threshold: {
      hitRatio: 0.75,
  ...
  },
  {
    ...
    statusPrefix: "STRICT",
    description:
      "Strict rollout requires <= 1% fallback anomaly, zero P0/P1 incidents, and two-window promotion evidence",
...
async function runGateTest(rootDir, gate) {
  ...
  const command = `pnpm exec vitest run --reporter=json --outputFile ".tmp/ghostframes/gates/${gate.name.toLowerCase()}.json" ${gate.testFile}`;
  ...
  const passedTests =
    passedTestsFromReport.length > 0 ? passedTestsFromReport : passedTestsFromStdout;
  const failedTests =
    failedTestsFromReport.length > 0 ? failedTestsFromReport : failedTestsFromStdout;
