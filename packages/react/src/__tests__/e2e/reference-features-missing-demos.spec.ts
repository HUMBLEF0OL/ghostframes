...
test.describe("Reference features missing demos", () => {
    ...
    test("resolver policy demo visibly reports selected mode and resolution outcome", async ({ page }) => {
        ...
        await expect(page.getByTestId("resolver-event-count")).toContainText(/^[1-9]\d* events captured$/);
    ...
    test("rollout telemetry collector visualizes non-empty snapshots and ratios", async ({ page }) => {
        await page.goto("/reference/features/fallback");
        await page.getByRole("button", { name: /start loading/i }).click();
        await page.getByRole("button", { name: /stop loading/i }).click();
        ...
        await expect(page.getByTestId("telemetry-snapshot-count")).toContainText(/^Snapshots:\s*[1-9]\d*$/);
        await expect(page.getByTestId("telemetry-latest-route")).toContainText(/\/reference\//i);
        await expect(page.getByTestId("telemetry-manifest-hit-ratio")).toContainText(/^Manifest hit ratio:\s*\d+%$/);
        await expect(page.getByTestId("telemetry-fallback-ratio")).toContainText(/^Fallback ratio:\s*\d+%$/);
    });

    test("manifest validation demo accepts valid input and rejects invalid input", async ({ page }) => {
        ...
        await expect(page.getByTestId("manifest-parse-failure")).toHaveText("Invalid manifest rejected");
