...
type StrictCompatibilityProfile = Omit<
  CompatibilityProfile,
  "requiredFields" | "allowedPolicies"
> & {
...
function resolveBasePolicy(input: {
  ...
  const isShadowRoute =
    input.shadowEnabled === true && matchesPrefix(input.pathname, input.shadowPaths ?? []);
  if (isShadowRoute) {
...
export function evaluateStrictRolloutSlo(input: {
  ...
  const rollbackRecommended =
    fallbackAnomalyRate > thresholds.rollbackFallbackAnomalyRate ||
    p0Incidents > 0 ||
    p1Incidents > 0;
