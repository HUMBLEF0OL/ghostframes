...
describe("runDiffCommand", () => {
    ...
    it("writes JSON diff output", async () => {
        ...
        await fs.writeFile(candidatePath, JSON.stringify(createManifest(["ProductCard"]), null, 2), "utf8");
...
async function mkTmpDir(): Promise<string> {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "skelcore-diff-test-"));
